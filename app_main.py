...
def build_models():
    ...
    if XGBClassifier is not None:
        models["XGBoost"] = XGBClassifier(eval_metric="logloss", random_state=42, n_estimators=60, max_depth=5, learning_rate=0.08)
    return models
...
def hero(df_train, df_test):
    ...
    st.markdown(
        ...
        <section class="hero">
            ...
            <p>Upload transaction data, compare models, test predictions, and check the energy use of each algorithm.</p>
...
def render_algorithm_emissions(results_df):
    section("Carbon footprint by algorithm", "This section shows the estimated carbon emission for each algorithm in a simple way.", "Algorithm Carbon")
...
def render_curves(training_sets, df_test, dataset_name):
    section("ROC and precision-recall curves", "These graphs help compare how well the models separate fraud from normal transactions.", "Advanced Evaluation")
...
def render_feature_importance(training_sets, df_test, dataset_name):
    section("Feature importance", "This shows which features have the biggest effect for the selected tree-based model.", "Explainability")
    ...
    st.markdown(
        ...
        <div class="landing-grid">
            <div class="card">
                ...
                <h3>A Financial fraud detection project</h3>s
                <p>This dashboard brings together fraud analysis, model comparison, prediction testing, and carbon tracking in one place.</p>
            </div>
            <div class="card">
                <div class="eyebrow">Main Features</div>
                <p>You can compare algorithms, inspect feature importance, test predictions, and look at carbon emission results.</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_methodology():
    section("How the system works", "This section explains how the models are trained and how the carbon values are estimated.", "Methodology")
    st.markdown(
        """
        <div class="method-grid">
            <div class="card">
                <div class="eyebrow">Fraud Modeling</div>
                <p>The app reads the uploaded datasets, creates useful time-based features, and compares several machine learning models on the same data.</p>
            </div>
            <div class="card">
                <div class="eyebrow">Two-Stage Flow</div>
                <p>The first stage filters transactions quickly, and the second stage checks the suspicious ones more carefully.</p>
            </div>
            <div class="card">
                <div class="eyebrow">Carbon Estimation</div>
                <p>The carbon values are estimated from runtime, device power, utilization, and grid intensity. If CodeCarbon is available, measured values can also be shown.</p>
            </div>
            <div class="card">
                <div class="eyebrow">How To Read The Results</div>
                <p>The best model should not be judged only by accuracy. It is better to look at performance, speed, and carbon emission together.</p>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with st.sidebar:
    st.markdown("## Inputs")
    st.caption("Upload your datasets and choose the settings for the model comparison.")
    ...
    limit = st.slider("Benchmark sample size", 1000, 30000, 5000, 1000)
...
if not valid(df_train, "Training dataset") or not valid(df_test, "Test dataset"):
    st.markdown('<div class="empty"><div class="eyebrow">Get Started</div><h3>Upload the training and test CSV files</h3><p>After that, the app will compare models, show predictions, and display carbon results.</p></div>', unsafe_allow_html=True)
    st.stop()
...
views = ["Command Center", "Model Lab", "Predictions", "Insights", "Sustainability", "Data Quality", "Trends", "Methodology"]
current_view = st.sidebar.radio("Section", views, key="workspace_view")

results_df = None
global_best = None
global_low = None
views_needing_benchmarks = {"Command Center", "Model Lab", "Predictions", "Insights", "Sustainability"}
if current_view in views_needing_benchmarks:
...
if current_view == "Command Center":
    section("Dataset overview", "This section gives a quick summary of the uploaded data and class balance.", "Overview")
    ...
    section("Alerts", "These messages highlight possible issues in the current results.", "Monitoring")
    render_alerts(df_train, df_test, results_df)
    section("Two-stage detection", "The first model filters likely normal transactions and the second model checks suspicious ones.", "Two-Stage Flow")
    ...
    stage_limit = min(limit, len(X_train), len(X_test))
    if pd.Series(y_train).nunique() < 2:
        st.info("The two-stage pipeline needs at least two classes in the selected training dataset.")
    elif stage_limit == 0:
        st.info("Not enough rows are available to run the two-stage pipeline.")
    else:
        X_train_stage, y_train_stage = sample_dataset(X_train, y_train, stage_limit)
        X_test_stage, y_test_stage = sample_dataset(X_test, y_test, stage_limit)
        stage1, stage2 = train_two_stage_model(X_train_stage, y_train_stage)
        final_pred, suspicious_mask = predict_two_stage(stage1, stage2, X_test_stage)
        m2 = st.columns(4)
        ...
        m2[3].metric("Final Fraud Recall", f'{classification_report(y_test_stage, final_pred, output_dict=True, zero_division=0).get("1", {}).get("recall", 0):.3f}')
        st.caption(f"Using a {stage_limit:,}-row sampled evaluation slice for cloud-friendly performance.")
        st.pyplot(plot_confusion_matrix(get_confusion_matrix(y_test_stage, final_pred)), use_container_width=True)

elif current_view == "Model Lab":
    section("Model comparison", "Compare model performance, runtime, and carbon emission side by side.", "Model Lab")
    ...
    section("Check one transaction", "Pick a row from the test data and see how the selected model classifies it.", "Prediction Workbench")
    best = results_df.sort_values("F1 Score", ascending=False).iloc[0]
    ...
    if pd.Series(y_train).nunique() < 2:
        ...
        fit_limit = min(limit, len(X_train))
        X_train_fit, y_train_fit = sample_dataset(X_train, y_train, fit_limit)
        model = clone(build_models()[model_name])
    ...
    st.divider()
    section("Try your own input", "Enter a few transaction details and test them with the Stage 2 model.", "Manual Prediction")
    ...
    if st.button("Predict Fraud", key="manual_predict"):
        ...
        if pd.Series(manual_y_train).nunique() < 2:
            ...
            manual_limit = min(limit, len(manual_X_train))
            manual_X_fit, manual_y_fit = sample_dataset(manual_X_train, manual_y_train, manual_limit)
            _, manual_stage2 = train_two_stage_model(manual_X_fit, manual_y_fit)
    ...
    section("Quick insights", "Ask simple questions about the results and get a short answer.", "Insights")
    ...
    section("Carbon and energy results", "This section shows estimated energy use and carbon emission for the current run.", "Sustainability")
    ...
    section("Data quality", "Review the uploaded datasets, missing values, and sample rows.", "Data Quality")
    ...
    section("Fraud trends", "View how fraud changes over time and how dataset size affects computation.", "Trend Intelligence")
    left, right = st.columns(2)
    with left:
